...
func startClient(t *testing.T, stub *pushStub, addrs ...string) (*Client, context.CancelFunc) {
	t.Helper()
	// An hour: these tests drive the refresher through notifications, so the
	// reconcile must not fire and confuse what caused a refresh. A test about
	// the reconcile itself uses startClientEvery.
	return startClientEvery(t, stub, time.Hour, addrs...)
}
