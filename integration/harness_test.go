...
// TTL, a txid mismatch, a broadcast cancelled while the node holds the
// reply, and the push model: a deposit the indexer pushes, a notification it
// drops, a reconnect, and one address it fails. The indexer role is played by
// a small in-test block scanner, and for the push scenarios by an in-process
// fake ElectrumX over that scanner (fake_electrumx_test.go), so the harness
// needs no ElectrumX build; the ElectrumX fork's wire behaviour is exercised
// by the electrumx package's protocol tests.
package integration
