...
## Provenance: which parts are production-proven, and which are not

This distinction is worth stating precisely, because an earlier version of this
README did not, and the imprecision hid two serious defects.
...
**The transaction construction and signing layer was not.** The `tx` package is a
separate implementation that diverged from the production signer, and up to
`v0.3.1` it was broken in two ways at once: it serialized outputs with two
extension bytes consensus had removed, and it shipped no witness assembly at all,
so the only signing example produced transactions every node rejected. The
production signer had the correct format throughout. The SDK did not inherit it.

Saying the SDK "was extracted from a service running in production" invited the
reading that the signing path had been exercised there. It had not, and the
divergence was the defect.

That path is now proven directly rather than by association: a transaction built,
signed, serialized, broadcast and confirmed by this SDK, with the identifiers to
decode it and the steps to reproduce it, is recorded in
[Verification](docs/VERIFICATION.md). Serialization is additionally pinned against
golden vectors generated by the production signer, so the two cannot drift again
